...
answers = ['Я не понял, что ты хочешь сказать.',
           'Извини, я тебя не понимаю.',                                                                # Примеры ответов бота на фото
...
def welcome(message):
    ...
    button2 = types.KeyboardButton('⚙️ Настройки')                                                      # Создание кнопок на главном меню
...
def info(message):
    if message.text == '🛍 Товары':
        ...
        settingsChapter(message)                                                                        # Обработка обычных текстовых команд, описанных в кнопках
        ...
        bot.send_message(message.chat.id, 'Гейнер BIGSNT BIG GAINER для набора мышечной массы высокобелковый, '
                                          ...
                                          'В одной порци: 43,4 белки, 9 жиры, 74 углеводы, 550 ккал.'
                                          'Цена: 1500 рублей.', reply_markup=markup)
        ...
        bot.send_message(message.chat.id, 'Bombbar Протеиновые батончики в шоколаде без сахара '
                                          ...
                                          'Цена: 800 рублей.', reply_markup=markup)
    elif message.text == 'Bombbbar proteinbar2':
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        button2 = types.KeyboardButton('↩️ Назад')
        button1 = types.KeyboardButton('💳 Купить')
        markup.row(button1, button2)
        ...
        bot.send_message(message.chat.id, '/start для перезапуска бота')
    elif message.text == '⚙️ Настройки #2':                                                             # Не придумал настройки
        ...
        bot.send_message(message.chat.id, answers[random.randint(0, 3)])                           # Рандомные ответы на фото
...
def proteinbarChapter(message):
    ...
    button2 = types.KeyboardButton('Bombbbar proteinbar2')
...
def settingsChapter(message):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)                                               # Функция, отвечающая за раздел настроек
